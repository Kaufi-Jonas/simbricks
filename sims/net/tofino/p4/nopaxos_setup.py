...
class l2_switch():
    ...
    def learning_cb(dev_id, pipe_id, direction, parser_id, session, msg):
        ...
        for digest in msg:
            ...
            print('MAC: {},  Port={}'.format(mac(mac_addr), port), end='')
            ...
            smac.entry_with_smac_hit(
                src_addr=mac_addr, port=port, is_static=False, ENTRY_TTL=60000
            ).push()
            dmac.entry_with_dmac_unicast(dst_addr=mac_addr, port=port).push()
        return 0

    def l2_add_smac_drop(self, vid, mac_addr):
        mac_addr = mac(mac_addr)
        self.p4.Ingress.smac.entry_with_smac_drop(src_addr=mac_addr).push()

...
class l2_switch():
    ...
    def clear_all(self, verbose=True, batching=True, clear_ports=False):
        ...
        for table_types in (['MATCH_DIRECT',
                             'MATCH_INDIRECT_SELECTOR'], ['SELECTOR'],
                            ['ACTION_PROFILE'], ['PRE_MGID'], ['PRE_ECMP'],
                            ['PRE_NODE'], []):  # This is catch-all
            for table in list(table_list):
                if table['type'] in table_types or len(table_types) == 0:
                    try:
                        if verbose:
                            print(
                                'Clearing table {:<40} ... '.format(
                                    table['full_name']
                                ),
                                end='',
                                flush=True
                    ...
                    if use_entry_list:
                        try:
                            ...
                            entry_list = table['node'].get(
                                regex=True, return_ents=True, print_ents=False
                            )
    ...
    def __init__(self, default_ttl=60000):
        ...
        self.vlan = {}
        self.all_ports = [
            port.key[b'$DEV_PORT'] for port in
            bfrt.port.port.get(regex=1, return_ents=True, print_ents=False)
        ]
        self.l2_age_ttl = default_ttl
        if (
            hasattr(self.p4.Egress, 'port_vlan_member') and
    ...
    def setup(self):
        ...
        for dp in self.all_ports:
            ...
            self.p4.IngressParser.PORT_METADATA.entry(
                ingress_port=dp, port_vid=0, port_pcp=0, l2_xid=l2_xid
            ).push()
        ...
        self.p4.Ingress.smac.idle_table_set_notify(enable=False, callback=None)

        self.p4.Ingress.smac.idle_table_set_notify(
            enable=True,
    ...
    def learning_cb(dev_id, pipe_id, direction, parser_id, session, msg):
        ...
        for digest in msg:
            ...
            print(
                'VID: {},  MAC: {},  Port={}'.format(vid, mac(mac_addr), port),
                end=''
            )
...
+------+------------------------------------------"""
    ...
    def vlan_port_add(self, vid, dp, tagged=False):
        ...
        vlan_node = bfrt.pre.node.get(
            MULTICAST_NODE_ID=vid, return_ents=True, print_ents=False
        )
        ...
        if self.egr_vlan_port:
            ...
            print(
                'port_vlan_member.mod({}/{}, 1, {}) ... '.format(
                    vid, port, pipe
                ),
                end='',
                flush=True
            )
            self.p4.Egress.port_vlan_member.mod(vid << 7 | port, 1, pipe=pipe)
    ...
    def port_vlan_default_set(self, dp, vid):
        e = self.p4.IngressParser.PORT_METADATA.get(
            ingress_port=dp, return_ents=True, print_ents=False
        )
    ...
    def l2_lookup(self, vid, mac_addr):
        dmac_entry = self.p4.Ingress.dmac.get(
            vid=vid, dst_addr=mac_addr, return_ents=True, print_ents=False
        )
        if dmac_entry == -1:
            dmac_entry = None

        smac_entry = self.p4.Ingress.smac.get(
            vid=vid, src_addr=mac_addr, return_ents=True, print_ents=False
        )
        if smac_entry == -1:
            smac_entry = None
    ...
    def l2_add_smac_drop(self, vid, mac_addr):
        ...
        self.p4.Ingress.smac.entry_with_smac_drop(vid=vid,
    ...
    def l2_show(self, from_hw=True):
        dmac_entries = {}
        try:
            for e in self.p4.Ingress.dmac.get(
                regex=1, from_hw=from_hw, print_ents=False, return_ents=True
            ):
                dmac_entries[
        ...
        smac_entries = {}
        try:
            for e in self.p4.Ingress.smac.get(
                regex=1, from_hw=from_hw, print_ents=False, return_ents=True
            ):
                smac_entries[
